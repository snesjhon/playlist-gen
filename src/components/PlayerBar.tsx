...
export function PlayerBar({ song, isPlaying, onPlayPause }: Props) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-player-bg backdrop-blur-lg border-t border-border z-40 animate-[slideUp_0.3s_ease-out]">
      <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
        ...
        <button
          onClick={onPlayPause}
          className="w-9 h-9 rounded-full bg-play-btn text-play-btn-text flex items-center justify-center flex-shrink-0 hover:opacity-90 transition-colors cursor-pointer text-base"
        >
          {isPlaying ? '❚❚' : '▶'}
        </button>
      </div>
